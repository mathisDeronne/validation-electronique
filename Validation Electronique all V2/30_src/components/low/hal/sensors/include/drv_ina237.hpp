...
// PURPOSE: Lecture/écriture registres INA237 via ESP-IDF I2C directement
//          (pas de wrapper drv_i2c — ESP-IDF fournit déjà le driver I2C)
...
// Driver du capteur de courant INA237.
// Appelle directement les fonctions I2C d'ESP-IDF (driver/i2c.h).
class DrvIna237 {
public:
    explicit DrvIna237(uint8_t i2c_addr);
...
private:
    ...
    common::Status i2cWriteReg(uint8_t reg, uint16_t value);
    common::Status i2cReadReg(uint8_t reg, uint16_t& out_value);
    bool i2cPing();
